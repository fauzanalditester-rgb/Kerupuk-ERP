...
export default function CRM() {
  const { customers, addCustomer, salesOrders } = useERP();
                          ...
                          {orders.map(so => (
                            <tr key={so.id} className="border-t border-slate-100">
                              ...
                              <td className="px-4 py-2.5">
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${so.status === 'Completed' ? 'bg-emerald-50 text-emerald-700' :
                                  so.status === 'Processing' ? 'bg-amber-50 text-amber-700' :
                                    'bg-blue-50 text-blue-700'
                                  }`}>
                                  {so.status === 'Completed' ? 'Selesai' : so.status === 'Processing' ? 'Diproses' : 'Dikirim'}
                                </span>
                              </td>
                            </tr>
                          ))}
      ...
      >
        {selectedCustomer && (
          <form
            onSubmit={(e) => {
              e.preventDefault();
              // Note: Since context doesn't have updateCustomer, we just close for now
